...
class NodeGroup(object):
    def __init__(self, level):
        ...
        self.y_values = np.asarray(
            [64, 128, 176, 224, 256, 272, 288, 320, 368, 416, 464, 512])
        self.node_dic = self.generateNodePositions(self.y_values)
    ...
    def generateNodePositions(self, y_values):
        dic = {y: [] for y in y_values}
        for (x, y) in self.nodesLUT.keys():
            dic[y].append(x)
        return dic

    def getNodeFromPosition(self, x, y):

        y_i = (np.abs(self.y_values - y)).argmin()
        x_i = (
            np.abs(np.asarray(self.node_dic[self.y_values[y_i]]) - x)).argmin()

        return self.getNodeFromPixels(self.node_dic[self.y_values[y_i]][x_i], self.y_values[y_i])
